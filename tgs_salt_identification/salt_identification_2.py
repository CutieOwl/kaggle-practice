...
#args = parser.parse_args("--optimizer sgd".split())
...
if os.path.isdir(os.path.join(result_dir, "8.14-tgs-salt_%s_batch%s_epoch%s2" % (optimizer, BATCH_SIZE, EPOCHS))) == False:
    os.mkdir(os.path.join(result_dir, "8.14-tgs-salt_%s_batch%s_epoch%s2" % (optimizer, BATCH_SIZE, EPOCHS)))
model_dir = os.path.join(result_dir, "8.14-tgs-salt_%s_batch%s_epoch%s2" % (optimizer, BATCH_SIZE, EPOCHS))
...
print("8.14-tgs-salt_%s_batch%s_epoch%s2" % (optimizer, BATCH_SIZE, EPOCHS))
...
b6d = dense_block(b5t, bn_axis, 256)
...
results = saltModel.fit(X_train_augm,
                    ...
                    batch_size=BATCH_SIZE,
                    epochs=EPOCHS,
                    callbacks=[reducelrer, checkpointer])
...
sub.to_csv(os.path.join(model_dir, 'saltSubmission_8.14_%s_batch%s_epoch%s2.csv' % (optimizer, BATCH_SIZE, EPOCHS)))

print('Prediction result saved as saltSubmission_8.14_%s_batch%s_epoch%s2.csv' % (optimizer, BATCH_SIZE, EPOCHS))
